...
### Critique before coding, per `CLAUDE.md` section 1 -- every claim re-derived, the reads-only boundary confirmed by reading the installed client's own source, not just its docs

Sections 1, 2 and 4 all checked out exactly: the JWT-minting search (`jsonwebtoken`/`jose`/`SignJWT`/`jwt.sign` across `lib/`, `app/`, `middleware.ts` -- zero matches), and the retry search (`retry`/`backoff` -- only the POS offline queue and comments in one historical-tooling file, confirmed by opening it). The "205 call sites" figure reproduced exactly once scoped to `app/` + `lib/`, excluding `scripts/` and tests.

**Section 3's concession -- "not reproduced, and 190 clean calls clear nothing" -- is correct in both directions, not overstated or understated.** The math checks out (190 clean calls at a ~1% real rate would happen by chance ~15% of the time) and I have first-hand corroboration the plan couldn't cite: **I hit this exact error myself, twice, in this same session**, running `findAll(Stock_Ledger): JWT issued at future` and `findAll(asset_depreciation_bands): JWT issued at future` from a script during the 53-order import -- before any writes, on a third machine, confirming this is not a Vercel-only or single-machine artifact.

**Section 5's reads-only boundary is the right call, and I agree with it rather than widening it.** The stated reason (a write that failed after reaching Postgres is indistinguishable from one that failed before it) actually only cleanly applies to network-level failures -- `PGRST303` specifically is an auth-time rejection that happens before any SQL runs, so retrying a *write* on `PGRST303` alone is arguably safe in principle. Considered widening the boundary to cover that one case and rejected it: verifying "PostgREST never executes a write before this specific auth check" holds across every RPC and connection-pooling edge case is a materially less certain guarantee than "never retry a write," and `CLAUDE.md` section 2's tier for purchases and orders sets the bar higher than "probably safe." Also swept for read-only RPC calls sitting outside `lib/sheets_db.ts`'s boundary that would be left unprotected -- found none (every `.rpc()` call site in `lib/` is one of the write-side `*-transaction.ts` files; the one preview-style read, `previewDisposalCharge`, already goes through plain `findAll`). **One real, adjacent gap found and deliberately not fixed, per the instruction not to widen silently:** `getStocktakeConfirmPreview` calls `applyStocktakeSessionAtomic` with `dryRun: true` -- read-like in effect, but it travels through the same RPC as the real write, so it sits outside this fix's boundary and would still break under the same transient error. Recorded here, not touched.

**Found while designing the retry-matcher, not stated in the plan: `PGRST303` itself carries HTTP 401** -- the same status class as a genuinely wrong key -- confirmed by reading the installed `@supabase/postgrest-js` (2.108.2) source directly rather than assuming. This means the plan's own "do not retry on 401/403" instruction cannot be satisfied by checking the HTTP status at all; status alone cannot distinguish the two 401 cases. The retry matcher is an **allowlist** (`error.code === "PGRST303"`, or its message, or a client-side network failure) rather than a denylist on status, which is the only way both halves of the requirement can be true at once. Also found, same source-reading pass: `@supabase/postgrest-js` already retries some transient failures internally (503/520, GET/HEAD/OPTIONS only, 3 attempts) -- a different, non-overlapping class from the JWT-skew case, so it does not already solve this and does not conflict with the adapter-level retry added here.

### The fix

`lib/sheets_db.ts`: `withReadRetry` (2 retries, 150ms then 400ms, per the plan's own numbers), wired into all four read helpers -- `findAllNoCache`, `findAllWhere`, `findById`, `getHeadersNoCache` (the last one not named in the plan's text but covered as "any other read helper," since it shares the identical shape). Each call site's query-building was moved into a factory closure passed to `withReadRetry`, so a retry issues a genuinely fresh request rather than re-awaiting an already-resolved query object -- confirmed by reading `PostgrestBuilder`'s `then()` that re-awaiting would in fact reissue the fetch today, but built defensively rather than relying on that specific, unstated behaviour surviving a future client upgrade. Every retry logs the table/context name via `console.warn`, asserted by a test, not by reading the code (section 7's own requirement). No write path (`insert`, `insertMany`, `update`, `updateMany`, `remove`, `removeMany`) was touched -- confirmed by diff, not by intent.

### Verification

Two tests, exactly as required. `lib/sheets_db.test.ts`'s new "retries once on PGRST303" case **proven to fail first** against the pre-fix code -- failed on the **value** (the real thrown `Error: findAll(Base_Ingredients): JWT issued at future`), not on a missing function; `findAllNoCache` already existed, only its behaviour was wrong. The second, equally required case (a 401 lacking the `PGRST303` signature) already passed on the pre-fix code trivially, since there was no retry logic at all yet to exercise -- the fix had to keep it passing, not make it pass.

### Gates

`tsc` 0 errors; `vitest` 216 files / 1495 tests (+2); `check-rules-current` exits 0 (unchanged: 3 PASS, 1 WARN, `OPEN-ITEMS 58`); `npm run build` succeeds. `OPEN-ITEMS 66` updated to record the mitigation landed and explicitly say it is not closed. **Not pushed.**
...
**Classification pass sent for review before converting anything, as instructed** (bigger than the Stocktake pass). Went through all 20 `it` blocks. Two corrections came back on the draft read, both applied: (1) the two-column layout (`lg:grid-cols-2`) is genuinely (b) -- breakpoint-conditional, jsdom evaluates no media queries -- but "compact field sizing" (`w-24 shrink-0`, no `<textarea>`) was wrongly grouped with it; those are unconditional, single-state DOM facts, not breakpoint claims, so they convert to (a). (2) "does not compute on-hand itself" -- the strongest reframing in the pass -- converts to (a) by expressing it as behaviour: submit a quantity exceeding the mocked item's `onHand` and assert it reaches `createIssueSlip` unmodified (proving the client did not pre-empt *this input*), then assert the RPC's own rejection renders verbatim. The test name and a comment both say explicitly that this proves non-pre-emption for that one input, not that no local check of any shape exists anywhere -- the old regex only forbade one spelling of a local comparison. 18 of 20 converted; 1 (`lg:grid-cols-2`) stays source-text; 1 ("uses SearchableSelect") deleted outright as redundant, since every converted test that selects an item necessarily drives the real `SearchableSelect` combobox to do it.

**Three conditions applied to every conversion, all about not letting a test name promise more than it delivers:** (1) class-presence-only assertions (tap targets, `w-24` field width) say so in the test's own `describe`/`it` name -- "class presence, not measured size" -- not only in a comment, since the name is what a run's output shows a future reader. (2) the on-hand test's limited scope (see above) is stated in its own name and a comment. (3) the two `confirm()`-mocking tests (backdated-month warning, reversal confirm) assert the actual **message content** passed to the mock -- "names Tháng 6/2026 through Tháng 8/2026", "names BR-INV-009 and that the original line is preserved" -- not merely that `confirm` was called, which the original source assertions actually claimed and a weaker render test would have under-delivered on.

**New tests in the existing `IssueSlipClient.test.tsx`** (one render file per component, not several): package-size counting into the RPC payload; the backdated-month confirm with real month names, both declined (blocks the RPC, using `vi.setSystemTime` for a deterministic month list) and approved (proceeds); the on-hand non-pre-emption case above; the datetime field defaulting near "now" and the submitted `issuedAtIso` carrying the typed instant, not midnight; add/remove line list; **one `createIssueSlip` call carrying all 3 lines of a 3-line slip**, not one per item; exactly one shared datetime/reason field regardless of line count; per-line validation naming the wrong line (all three message shapes: missing item, missing package size, zero quantity); reversal eligibility (button shown only for the genuinely eligible row among an eligible/is-a-reversal/already-reversed trio); the reversal confirm's message content; a reversed pair rendering both directions with neither row hidden; grouping by `slipId` with the legacy `(phiếu cũ)` fallback; reversal staying per-line (button count matches eligible-row count, not one per slip); the explicit empty state; and the M2/M3/M4 mobile checks (`inputMode`, tap-target classes, the live ready-to-submit count).

**`IssueSlipClient.test.ts` slimmed from 20 assertions to 1** -- the `lg:grid-cols-2` bucket-(b) case, with a header comment explaining why, matching the StocktakeClient pattern.

**Proven to have teeth on the one-RPC-call/validation path, per the explicit ask for a mutation that matters, not a cosmetic string.** `payloadLines.push({..., baseQuantity: parsedQty * pkg.conversionRate})` changed to `... + 1`. Three render tests failed with the exact wrong numbers: 36 -> 37 (single-line case), 999*12 = 11988 -> 11989 (the on-hand non-pre-emption case), and each line of the 3-line slip off by one (24/75/5 -> 25/76/6). Both old source-text assertions covering that exact line (`expect(source).toContain("parsedQty * pkg.conversionRate")`, and the `payloadLines.push(` / `lines: payloadLines` / single-`createIssueSlip(`-occurrence checks) still passed -- the mutated line still contains those substrings. Reverted; `git diff` on the component clean.

**Test count:** file total 29 -> 29 stays the same shape as Stocktake's pattern per-directory, but the suite total moved: 1169 -> 1176 (+7 net: -19 deleted source-text assertions, +26 new/expanded render tests across the file, +2 pre-existing unit-label tests carried over unchanged -- 28 total in `.test.tsx`, 1 left in `.test.ts`).

**OPEN-ITEMS 38 closed.** Both Plan D screens named in the item are now render-tested with only the genuinely CSS-inexpressible claims left as source text. What is explicitly NOT claimed: this item was never an audit of every component in the repo with the same source-text-only shape -- only Plan D's two named screens, both now done.

**Verified:** `tsc --noEmit` 0 errors. `vitest run` 1176/1176 pass. `check-rules-current` clean. `npm run build` succeeds.
