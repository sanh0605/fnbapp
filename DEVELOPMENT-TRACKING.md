...
**Verification:** `tsc --noEmit` clean and full suite green after each of the 3 commits (final: 757/757), `next build` passed. Confirmed via `supabase migration list` that `0036`/`0037` are already applied to production but `0038` is not. **All 3 commits held back from push and production** — deploying the code change ahead of the migration would break both stock-read paths against a table that doesn't exist yet. Waiting on the owner to run `supabase db push`, after which the deferred plan Task 4 steps (push these commits, run `scripts/audit-inventory-balances.ts` + `scripts/audit-current-stock.ts` + `scripts/audit-pnl-mac-consistency.ts` live) can proceed.
