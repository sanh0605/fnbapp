...
**Not committed/pushed yet** -- migration 0034 was pushed to the database (owner approved), but the local git commit for `lib/full-history-recompute.ts`, its test file, the new migration, and the new script has not been made yet; pending owner's next instruction.
