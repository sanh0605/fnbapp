...
describe("getPurchaseOrdersData", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("propagates the failure instead of returning a fabricated empty result", async () => {
    ...
    await expect(getPurchaseOrdersData()).rejects.toThrow("db down");
  });

  it("genuinely empty tables still resolve with [] and do not throw", async () => {
    mocks.findAll.mockResolvedValue([]);

    await expect(getPurchaseOrdersData()).resolves.toEqual({
      orders: [], suppliers: [], lines: [], items: [],
    });
  });
});
