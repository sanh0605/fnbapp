...
| `INV-LEGACY-SYNC` | Apply the former V1 inventory synchronization/correction workflow | Administrator | none (removed) | `REMOVED` | Owner asked (2026-07-20) whether the admin page was still needed after seeing it flag 631 false-positive-heavy discrepancies; execute action had already returned HTTP 410 since 2026-07-17, and the scan view was calling `auditOrderLedger` without the `recipes`/`semiProducts`/`shortfallCutoverAt` parameters the same-day BTP-shortfall work required, producing stale/misleading counts (real current count via `scripts/audit-order-ledger.ts` was 209) | Removed `/admin/inventory/sync/*`, `/api/inventory/sync/execute`, `/api/inventory/sync/scan`, and the sidebar link; no remaining entry point | No writes -- was already read-only | 2026-07-20; repository inspection | Claude review |
...
| `MAINT-INVENTORY-SCAN` | Scan V2 order-ledger discrepancies without applying a correction | Administrator and data/audit maintainer | none (removed) | `REMOVED` | Same 2026-07-20 removal as `INV-LEGACY-SYNC` -- superseded entirely by `scripts/audit-order-ledger.ts`, which is properly parameterized and actively maintained | This page's `auditOrderLedger` call never passed `recipes`/`semiProducts`/`shortfallCutoverAt`, so it always used the naive flat-recipe branch regardless of order date, producing a much larger and less accurate mismatch count than the real CLI audit | No writes -- was already read-only | 2026-07-20; repository inspection | Claude review |
