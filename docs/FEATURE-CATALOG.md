...
| `AUTH-SESSION-AUTHZ` | Restrict protected pages and sensitive actions by session and technical role | Owner and administrator; staff restricted to POS | `middleware.ts`; `resolveActor`; `requireAdmin`; protected `/admin/*` and `/pos/*` routes | `PARTIAL` | `middleware.ts`; `lib/auth.ts`; `lib/admin-auth-guard-audit.test.ts`; read-only `scripts/audit-admin-action-auth.ts` checked 19 action files | Middleware protects page routes and many writes have local guards, but the audit found `approveAndRecomputeAction` without an action-local guard; selected API/maintenance routes are outside the matcher; full RLS and direct-invocation testing belongs to Phase 3 | Sessions and all guarded business records | 2026-07-17; repository audit | Codex (server paths); Claude (access policy); Antigravity (UI) |
...
| `AUD-BACKDATE-REVIEW` | Review, reject, or explicitly recompute an approved backdated event | Owner approval; designated administrator/data maintainer | `/admin/audit/backdated-ledger`; recompute/reject actions and RPCs | `PARTIAL` | `lib/backdated-ledger/recompute.test.ts`; migrations `0015_backdated_event_recompute.sql`; read-only auth audit | Recompute engine is tested and atomic, but `approveAndRecomputeAction` lacks an action-local guard and trusts a supplied reviewer; current UI/operator walkthrough and notification are missing | Event status, selected line COGS, recovery change log | 2026-07-17; repository/tests and auth audit | Codex (recompute/data); Antigravity (UI); Claude review |
