...
Status: canonical contract; first evidence-backed inventory populated by Pre-Audit C

Last verified: 2026-07-17

## Tóm tắt cho chủ doanh nghiệp

Pre-Audit C đã lập danh sách chức năng dựa trên màn hình, luồng xử lý, bài kiểm tra, công cụ kiểm tra dữ liệu và bằng chứng vận hành hiện có. Trạng thái được ghi thận trọng: có màn hình nhưng thiếu kiểm chứng thực tế vẫn được xem là chưa xác minh; chức năng có giới hạn quan trọng được ghi là chỉ hoạt động một phần.

Hệ thống có nền tảng bán hàng, nhập hàng, tồn kho, giá vốn, báo cáo và sao lưu đã được kiểm chứng ở các phần cốt lõi. Các điểm cần ưu tiên xem xét tiếp gồm đổi mật khẩu còn dùng luồng dữ liệu cũ, một số thao tác bảo trì chưa có lớp bảo vệ đầy đủ, quyền truy cập chưa được kiểm tra toàn diện, phục hồi từ file sao lưu chưa được diễn tập, bán hàng khi mất mạng và mô hình nhiều chi nhánh chưa có.
...
The inventory below is a repository-and-evidence assessment, not the eight-gate system audit. `Repository/test environment` means the current checked-out code and the full automated suite on 2026-07-17. Production evidence is named explicitly when available. UI-only capabilities without a current operator walkthrough remain `LIVE_UNVERIFIED` even when their routes exist.
...
| `AUTH-SESSION-AUTHZ` | Restrict protected pages and sensitive actions by session and technical role | Owner and administrator; staff restricted to POS | `middleware.ts`; `resolveActor`; `requireAdmin`; protected `/admin/*` and `/pos/*` routes | `PARTIAL` | `middleware.ts`; `lib/auth.ts`; `lib/admin-auth-guard-audit.test.ts`; `app/admin/audit/backdated-ledger/actions.test.ts`; read-only `scripts/audit-admin-action-auth.ts` checked 19 action files | Backdated-ledger review mutations now enforce an action-local ADMIN session and derive the reviewer from that session; selected API/maintenance routes remain outside the matcher until SEC-3; full RLS and direct-invocation testing belongs to Phase 3 | Sessions and all guarded business records | 2026-07-18; repository/tests | Codex (server paths); Claude (access policy); Antigravity (UI) |
...
| Feature ID | Business capability | Intended users | Current entry points | Status | Evidence | Known limitations | Data affected | Last verified | Owner/maintainer |
|---|---|---|---|---|---|---|---|---|---|
| `POS-CATALOG` | Browse active products, variants, modifiers, promotions, and current inventory context while selling | Cashier/service staff, administrator | `/pos`; `app/pos/page.tsx`; `components/POSScreen.tsx` | `LIVE_UNVERIFIED` | Current POS route and data-loading code; responsive UI code exists | No current end-to-end operator walkthrough for catalog completeness, mobile usability, Vietnamese labels, or accessibility; multi-outlet scope is absent | Catalog reference data and compact inventory state | 2026-07-17; repository inspection | Antigravity (UI); Codex (data); Claude review |
| `POS-CHECKOUT` | Calculate totals, discounts, snapshots, COGS, inventory consumption, and save a completed sale atomically | Cashier/service staff, administrator | POS checkout in `/pos`; `submitOrderV2`; `savePosOrderAtomic` | `LIVE_VERIFIED` | `app/pos/actions.test.ts`; `lib/order-cart.test.ts`; `lib/order-math.test.ts`; `lib/pos-inventory-state.test.ts`; `lib/pos-order-transaction.test.ts`; `lib/pos-checkout-migration.test.ts`; production review `docs/audits/2026-07-02-pos-checkout-performance-review.md` | Online database access is required; notification delivery is not part of the atomic sale; one-shop scope only; current mobile operator regression is not recorded | `orders_v2`, `order_lines_v2`, `order_events`, `stock_ledger` | 2026-07-02 production; 2026-07-17 tests | Codex (transaction/data); Antigravity (UI); Claude review |
| `POS-DRAFTS` | Save, resume, list, and delete in-progress carts by brand | Cashier/service staff, administrator | POS draft controls; `getPOSDrafts`; `savePOSDraft`; `deletePOSDraft` | `LIVE_UNVERIFIED` | Current server actions in `app/pos/actions.ts`; `pos_drafts` is included in the production backup allowlist | No dedicated tests, operator walkthrough, conflict handling, expiry policy, or offline behavior; draft actions have no action-local session guard or actor/audit attribution | `pos_drafts` | 2026-07-17; repository inspection | Codex (data); Antigravity (UI); Claude review |
...
| Feature ID | Business capability | Intended users | Current entry points | Status | Evidence | Known limitations | Data affected | Last verified | Owner/maintainer |
|---|---|---|---|---|---|---|---|---|---|
| `ORD-LIST-DETAIL` | Search and inspect current and historical orders with line snapshots | Owner and administrator | `/admin/orders`; `getOrdersV2`; `getOrderDetailV2` | `LIVE_UNVERIFIED` | Current route, action, and snapshot coercion paths | No current UI/operator verification for filtering, large-volume behavior, export, mobile cards, or sensitive-field minimization; access depends mainly on route protection | `orders_v2`, `order_lines_v2`, catalog snapshots | 2026-07-17; repository inspection | Antigravity (UI); Codex (data); Claude review |
| `ORD-EDIT-SUPERSEDE` | Edit a completed order by creating a new version, reversing the old inventory effect, and preserving history | Owner and administrator | Order edit modal; `editOrderV2`; `supersedeOrderV2` | `PARTIAL` | `lib/order-edit-cart.test.ts`; `lib/sheets-db-v2-edit.test.ts`; `app/admin/orders/actions.test.ts`; `lib/order-ledger-audit.test.ts` | ADMIN guard, optimistic version check, snapshots, and actor fields exist, but persistence uses sequential writes with best-effort rollback rather than one database transaction; no production failure drill or current operator walkthrough | Orders, lines, events, reversal and consume ledger rows | 2026-07-17; repository/tests | Codex (transaction/data); Antigravity (UI); Claude review |
| `ORD-VOID` | Void a completed order with reason, reversal ledger, event trace, and actor attribution | Owner and administrator | Order detail; `voidOrderV2`; `scripts/audit-void-orders.ts` | `PARTIAL` | ADMIN guard and fail-safe/idempotency checks in `app/admin/orders/actions.ts`; `lib/order-ledger-audit.test.ts`; read-only audit script | Reversal, event, and final status are sequential rather than one atomic RPC; no dedicated current action test or production rollback drill; no notification/export behavior | `orders_v2`, `order_events`, `stock_ledger` | 2026-07-17; repository/tests | Codex (transaction/data); Antigravity (UI); Claude review |
...
| Feature ID | Business capability | Intended users | Current entry points | Status | Evidence | Known limitations | Data affected | Last verified | Owner/maintainer |
|---|---|---|---|---|---|---|---|---|---|
| `PROD-CATALOG-MASTER` | Manage product categories, products, variants, status, images, and prices | Owner and administrator | `/admin/products`; `/admin/products/categories`; `saveProduct`; category actions | `LIVE_UNVERIFIED` | Current routes and ADMIN-guarded mutations | No complete CRUD/operator test; product save spans multiple dependent writes without a single transaction; historical-reference deletion behavior and mobile/accessibility are not fully verified | `product_categories`, `products`, `product_variants`, `product_price_history` | 2026-07-17; repository inspection | Codex (data); Antigravity (UI); Claude review |
...
| Feature ID | Business capability | Intended users | Current entry points | Status | Evidence | Known limitations | Data affected | Last verified | Owner/maintainer |
|---|---|---|---|---|---|---|---|---|---|
| `PRICE-HISTORY` | Preserve variant price changes as effective-dated history | Owner and administrator; audit/report consumers | Product save action; `lib/price-history.ts` | `LIVE_UNVERIFIED` | `lib/price-history.test.ts`; price-history insertion in `app/admin/products/actions.ts`; migration parity artifacts | The timeline helper is tested, but the current write path has no integration/operator verification; there is no dedicated history review/export workflow; product save is not one atomic multi-table transaction | `product_variants`, `product_price_history` | 2026-07-17; repository/tests | Codex (data); Antigravity (UI); Claude review |
| `PROMO-ADMIN` | Create, edit, scope, activate, and delete promotions | Owner and administrator | `/admin/promotions`; `savePromotion`; `deletePromotionAction` | `LIVE_UNVERIFIED` | Current route and ADMIN-guarded mutations in `app/admin/promotions/actions.ts` | No CRUD integration test or operator walkthrough; delete is physical removal; historical sales rely on snapshots rather than the current promotion row; UI/accessibility unverified | `promotions` | 2026-07-17; repository inspection | Codex (data); Antigravity (UI); Claude review |
...
| `INV-STOCK-ADJUSTMENT` | Submit, approve, or reject stock corrections with reason and actor fields | Owner, administrator, inventory staff by intended policy | `/admin/inventory/stock-adjustments`; adjustment actions | `PARTIAL` | `resolveActor`/`requireAdmin` guards and reason requirement in `app/admin/inventory/actions.ts`; `scripts/audit-stock-adjustments.ts` | Adjustment and ledger writes are sequential rather than one atomic transaction; role mapping is broader/narrower than intended business roles; no action tests or current operator walkthrough | `stock_adjustments`, `stock_ledger` | 2026-07-17; repository inspection | Codex (transaction/data); Antigravity (UI); Claude review |
...
| `BTP-PRODUCTION-ORDER` | Record a production batch, consume ingredients, and add semi-product yield | Owner and administrator; production staff by intended policy | `/admin/production`; `saveProductionOrder`; `scripts/audit-production-stock.ts` | `PARTIAL` | Current route and ADMIN guard; production ledger audit script | Production order, item, consume rows, and yield row are inserted sequentially without one atomic transaction; no action test, rollback drill, or current operator walkthrough | `production_orders`, `production_items`, `stock_ledger` | 2026-07-17; repository inspection | Codex (transaction/data); Antigravity (UI); Claude review |

### 10. Revenue, COGS, and reports

| Feature ID | Business capability | Intended users | Current entry points | Status | Evidence | Known limitations | Data affected | Last verified | Owner/maintainer |
|---|---|---|---|---|---|---|---|---|---|
| `RPT-SALES` | Report sales totals and product/modifier detail with date, brand, and category filters | Owner and administrator | `/admin/reports/sales`; `getSalesDataV2` | `LIVE_VERIFIED` | `app/admin/reports/actions.test.ts` sales cases; `lib/report-time.test.ts`; order-math tests | No CSV/spreadsheet export or scheduled delivery; UI mobile/accessibility operator walkthrough is missing; access enforcement awaits Phase 3 | Read-only orders, lines, products, modifiers | 2026-07-17; repository/tests | Codex (report math/data); Antigravity (UI); Claude review |
| `RPT-PNL-MAC` | Report revenue, pinned MAC COGS, gross profit, and breakdowns without double counting | Owner and administrator | `/admin/reports/pnl`; `getPnLDataV2`; P&L audit scripts | `LIVE_VERIFIED` | `app/admin/reports/actions.test.ts`; `lib/report-v2-allocators.test.ts`; `lib/mac-cogs.test.ts`; `scripts/audit-pnl-mac-consistency.ts`; reviewed MAC audit artifacts | Stored COGS is authoritative while replay differences may be informational; no export/notification; malformed reference data can fail the report rather than silently guess | Orders, lines, snapshots, stock ledger, report aggregates | 2026-07-17; repository/tests and reviewed audits | Codex (financial engine/data); Antigravity (UI); Claude review |
| `RPT-HOURLY` | Show an hourly sales heatmap within the selected period | Owner and administrator | Sales report; `getHourlyHeatmapV2` | `LIVE_VERIFIED` | `app/admin/reports/actions.test.ts` verifies completed-status and UTC-range query; `lib/report-time.test.ts` | No current visual/operator regression, export, or notification; one-shop scope only | Read-only completed orders | 2026-07-17; repository/tests | Codex (query/data); Antigravity (UI); Claude review |
| `RPT-STOCK` | View current stock quantities and inventory status | Owner, administrator, inventory/production staff by intended policy | `/admin/reports/stock`; realtime stock action | `LIVE_UNVERIFIED` | Current route and ledger-derived stock action; stock audit scripts exist | No report-specific automated test/current operator walkthrough; physical stock is not certified by ledger balance; export and role scope are unverified | Stock ledger and inventory references | 2026-07-17; repository inspection | Codex (data); Antigravity (UI); Claude review |
| `RPT-PROMOTION-PERFORMANCE` | Compare promotion usage and attributed revenue/discount outcomes | Owner and administrator | Sales/P&L data actions; `getPromotionPerformanceV2` | `LIVE_UNVERIFIED` | Current server action and promotion/order snapshot data | No dedicated automated test, current UI entry confirmation, operator walkthrough, export, or historical-policy review | Orders, lines, promotion snapshots, promotions | 2026-07-17; repository inspection | Codex (report math/data); Antigravity (UI); Claude review |
...
| `USR-ROLE-ENFORCEMENT` | Enforce technical ADMIN/STAFF/SYSTEM roles while documenting intended business roles separately | Owner, administrator, cashier/service staff, inventory/production staff | `middleware.ts`; `resolveActor`; `requireAdmin`; admin action guards | `PARTIAL` | `lib/admin-auth-guard-audit.test.ts`; `app/admin/audit/backdated-ledger/actions.test.ts`; `scripts/audit-admin-action-auth.ts`; `docs/ACCESS-MODEL.md` | Current roles do not represent owner/inventory roles separately; the identified backdated-review action gap is closed, while selected API routes, RLS, session expiry/status changes, and broader direct invocation are not yet certified | Authorization decisions across all business data | 2026-07-18; repository/tests and audit | Claude (access policy); Codex (server enforcement); Antigravity (UI visibility) |
...
| `BKP-ADMIN-MANUAL` | Start a backup manually from the admin backup page | Owner and administrator | `/admin/backup`; `triggerBackup` | `PARTIAL` | Current ADMIN-guarded action in `app/admin/backup/actions.ts` | The page still calls legacy `backup-to-sheets`, not the approved Drive pull endpoint; no current operator verification; must not be treated as proof that the daily Drive backup ran | Legacy external backup destination, if deployed | 2026-07-17; repository inspection | Codex (backup path); Antigravity (UI); Claude review |
...
| Feature ID | Business capability | Intended users | Current entry points | Status | Evidence | Known limitations | Data affected | Last verified | Owner/maintainer |
|---|---|---|---|---|---|---|---|---|---|
| `EXT-TELEGRAM-ORDER` | Send an order summary to a configured Telegram chat | Owner/operations recipient | `supabase/functions/notify-order/index.ts` | `PARTIAL` | Edge Function source formats and sends Telegram messages | No caller exists in the current application, no automated test or operator evidence is recorded, delivery is not part of checkout, retry/idempotency and authentication/deployment settings are unverified | External message containing order summary fields | 2026-07-17; repository inspection | Codex (server integration); Claude review |

### 15. Settings and maintenance tools

| Feature ID | Business capability | Intended users | Current entry points | Status | Evidence | Known limitations | Data affected | Last verified | Owner/maintainer |
|---|---|---|---|---|---|---|---|---|---|
| `SET-PASSWORD` | Let a signed-in user change their own password | All signed-in users | `/settings/password`; `changePasswordAction` | `PARTIAL` | Current page/action; direct comparison with `lib/auth.ts` and user-admin actions | The action reads a legacy Google Sheet, expects `session.user.username` that the session callback does not set, and writes SHA-256 while login/admin user flows use Supabase bcrypt; no test/operator evidence; do not rely on this path until remediated | Legacy Sheets user password cell rather than the authoritative Supabase credential | 2026-07-17; repository inspection | Codex (auth/data remediation); Antigravity (UI); Claude security review |
| `MAINT-CACHE` | Clear or revalidate selected application caches | Administrator | `/admin/clear-cache`; `/api/revalidate` | `PARTIAL` | Current admin page and revalidation API route | The callable path exists, but the API route is outside the auth middleware and has no local secret/session check; no automated/operator verification; affected cache tags use legacy names; business impact of unauthenticated invocation is availability/staleness rather than data mutation | Next.js cache only | 2026-07-17; repository inspection | Codex (server path); Antigravity (UI); Claude security review |
| `MAINT-INVENTORY-SCAN` | Scan V2 order-ledger discrepancies without applying a correction | Administrator and data/audit maintainer | `/admin/inventory/sync`; `/api/inventory/sync/scan`; `auditOrderLedger` | `PARTIAL` | `lib/order-ledger-audit.test.ts`; current read-only scan endpoint; execute endpoint is retired with HTTP 410 | Scan endpoint is outside auth middleware and can expose order/inventory discrepancy metadata; no current operator walkthrough; there is deliberately no automatic correction or export | Read-only orders, lines, stock ledger, inventory names | 2026-07-17; repository/tests | Codex (audit/data); Antigravity (UI); Claude security review |
