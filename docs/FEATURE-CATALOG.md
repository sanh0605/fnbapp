...
| `AUTH-SESSION-AUTHZ` | Restrict protected pages and sensitive actions by session and technical role | Owner and administrator; staff restricted to POS | `middleware.ts`; `resolveActor`; `requireAdmin`; protected `/admin/*` and `/pos/*` routes | `PARTIAL` | Gate 2 read-only map `docs/audits/2026-07-18-gate2-access-map.md`; `lib/admin-auth-guard-audit.test.ts`; Gate 1 rejection tests | Current map covers 21 Server Action files/81 exports and 4 API files/5 handlers. It found 4 mutation access gaps and 21 unguarded reads; RLS and session lifecycle remain open | Sessions and all guarded business records | 2026-07-18; repository/tests and audit | Codex (server paths); Claude (access policy); Antigravity (UI) |
...
| `POS-CHECKOUT` | Calculate totals, discounts, snapshots, COGS, inventory consumption, and save a completed sale atomically | Cashier/service staff, administrator | POS checkout in `/pos`; `submitOrderV2`; `savePosOrderAtomic` | `LIVE_VERIFIED` | Transaction/math tests and production review remain valid; Gate 2 access map inspected `submitOrderV2` directly | Financial persistence is verified, but direct invocation without a session is not rejected and is attributed to SYSTEM; this access gap requires a separately reviewed fix. Online access and one-shop scope remain limitations | `orders_v2`, `order_lines_v2`, `order_events`, `stock_ledger` | 2026-07-18; repository/tests and access audit | Codex (transaction/data); Antigravity (UI); Claude review |
| `POS-DRAFTS` | Save, resume, list, and delete in-progress carts by brand | Cashier/service staff, administrator | POS draft controls; `getPOSDrafts`; `savePOSDraft`; `deletePOSDraft` | `LIVE_UNVERIFIED` | Gate 2 access map inspected all three draft actions; `pos_drafts` is included in the production backup allowlist | List/save/delete lack a rejecting local session guard; save can attribute an unauthenticated caller to SYSTEM. Tests, operator walkthrough, conflict handling, and expiry policy are also missing | `pos_drafts` | 2026-07-18; repository access audit | Codex (data); Antigravity (UI); Claude review |
...
| `INV-STOCK-ADJUSTMENT` | Submit, approve, or reject stock corrections with reason and actor fields | Owner, administrator, inventory staff by intended policy | `/admin/inventory/stock-adjustments`; adjustment actions | `PARTIAL` | Gate 2 access map; current `resolveActor`/`requireAdmin` guards; `scripts/audit-stock-adjustments.ts` | Submit rejects anonymous callers but accepts STAFF on direct invocation even though cashier access is not intended; the future Inventory role is unresolved. Writes are also sequential and lack action/operator verification | `stock_adjustments`, `stock_ledger` | 2026-07-18; repository access audit | Codex (transaction/data); Antigravity (UI); Claude review |
...
| Feature ID | Business capability | Intended users | Current entry points | Status | Evidence | Known limitations | Data affected | Last verified | Owner/maintainer |
|---|---|---|---|---|---|---|---|---|---|
| `USR-ADMIN` | Create, edit, delete, and reset technical ADMIN/STAFF users | Owner and administrator | `/admin/users`; user actions; `user-admin` Edge Function | `LIVE_UNVERIFIED` | ADMIN mutation guards and safe projections have tests; Gate 2 reviewed both app actions and Edge Function source | SEC-1 is closed. Two app read actions still lack local ADMIN guards; normal Edge routes have a local owner check, while `/migrate` depends on platform JWT signature verification that is not deployment-verified | `users` and active sessions | 2026-07-18; repository/tests and access audit | Codex (server/data); Antigravity (UI); Claude security coordination |
| `USR-ROLE-ENFORCEMENT` | Enforce technical ADMIN/STAFF/SYSTEM roles while documenting intended business roles separately | Owner, administrator, cashier/service staff, inventory/production staff | `middleware.ts`; `resolveActor`; `requireAdmin`; admin action guards | `PARTIAL` | `docs/audits/2026-07-18-gate2-access-map.md`; 10 focused audit-helper tests; Gate 1 rejection tests; `docs/ACCESS-MODEL.md` | Gate 2 found 25 local action gaps, including unauthenticated SYSTEM attribution and one wrong-role inventory mutation; RLS, session lifecycle, and future Inventory role remain unresolved | Authorization decisions across all business data | 2026-07-18; repository/tests and audit | Claude (access policy); Codex (server enforcement); Antigravity (UI visibility) |
...
| `BKP-ADMIN-MANUAL` | Start a backup manually from the admin backup page | Owner and administrator | `/admin/backup`; `triggerBackup` | `PARTIAL` | Gate 2 confirms the action-local ADMIN guard; backup policy verifies the separate daily Drive path | The button still calls legacy `backup-to-sheets`; that function has no local caller authorization and its deployed platform-JWT setting is unverified. It is not proof that daily Drive backup ran | Legacy external backup destination, if deployed | 2026-07-18; repository access audit | Codex (backup path); Antigravity (UI); Claude review |
