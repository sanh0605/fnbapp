...
| `AUTH-SESSION-AUTHZ` | Restrict protected pages and sensitive actions by session and technical role | Owner and administrator; staff restricted to POS | `middleware.ts`; `resolveActor`; `requireAdmin`; protected `/admin/*` and `/pos/*` routes | `PARTIAL` | Gate 2 map plus Wave 1 regression tests in `app/pos/actions.auth.test.ts` and `app/admin/inventory/actions.auth.test.ts` | All 60 mutations now have matching local gates and POS SYSTEM attribution is CLI-only; 20 admin read actions, RLS, and session lifecycle remain open | Sessions and all guarded business records | 2026-07-18; repository/tests and audit | Codex (server paths); Claude (access policy); Antigravity (UI) |
...
| `INV-STOCK-ADJUSTMENT` | Submit, approve, or reject stock corrections with reason and actor fields | Owner and administrator | `/admin/inventory/stock-adjustments`; adjustment actions | `PARTIAL` | Wave 1 regression tests verify ADMIN-only submission, immediate approval, and ledger creation; `scripts/audit-stock-adjustments.ts` | Policy narrowed on 2026-07-18: STAFF can no longer submit adjustments. Historical PENDING rows remain reviewable; writes are sequential and operator verification is missing | `stock_adjustments`, `stock_ledger` | 2026-07-18; repository tests and access audit | Codex (transaction/data); Antigravity (UI); Claude review |
...
| Feature ID | Business capability | Intended users | Current entry points | Status | Evidence | Known limitations | Data affected | Last verified | Owner/maintainer |
|---|---|---|---|---|---|---|---|---|---|
| `USR-ADMIN` | Create, edit, delete, and reset technical ADMIN/STAFF users | Owner and administrator | `/admin/users`; user actions; `user-admin` Edge Function | `LIVE_UNVERIFIED` | ADMIN mutation guards and safe projections have tests; Wave 1 rejects a forged service-role payload and accepts only the exact runtime service key for `/migrate` | SEC-1 and the local `/migrate` token gap are closed. Two app read actions still lack local ADMIN guards, normal Edge-route deployment JWT settings are unverified, and operator/session-invalidation checks remain open | `users` and active sessions | 2026-07-18; repository/tests and access audit | Codex (server/data); Antigravity (UI); Claude security coordination |
| `USR-ROLE-ENFORCEMENT` | Enforce technical ADMIN/STAFF/SYSTEM roles while documenting intended business roles separately | Owner, administrator, cashier/service staff, inventory/production staff | `middleware.ts`; `resolveActor`; `requireAdmin`; admin action guards | `PARTIAL` | Gate 2 map; Wave 1 direct-rejection tests; 10 focused audit-helper tests; Gate 1 tests; `docs/ACCESS-MODEL.md` | All 60 mutations now have matching local gates and POS SYSTEM is CLI-only; 20 admin reads, RLS, session lifecycle, and future Inventory role remain unresolved | Authorization decisions across all business data | 2026-07-18; repository/tests and audit | Claude (access policy); Codex (server enforcement); Antigravity (UI visibility) |
