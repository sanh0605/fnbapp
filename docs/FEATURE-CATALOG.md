...
| `AUTH-SESSION-AUTHZ` | Restrict protected pages and sensitive actions by session and technical role | Owner and administrator; staff restricted to POS | `middleware.ts`; `resolveActor`; `requireAdmin`; protected `/admin/*` and `/pos/*` routes | `PARTIAL` | Gate 2 map; Wave 1 and Wave 2 direct-rejection tests; comprehensive action-guard audit | All 83 action exports now have matching local gates, full admin reads are ADMIN-only, and POS SYSTEM attribution is CLI-only; RLS and session lifecycle remain open | Sessions and all guarded business records | 2026-07-18; repository/tests and audit | Codex (server paths); Claude (access policy); Antigravity (UI) |
...
| Feature ID | Business capability | Intended users | Current entry points | Status | Evidence | Known limitations | Data affected | Last verified | Owner/maintainer |
|---|---|---|---|---|---|---|---|---|---|
| `ORD-LIST-DETAIL` | Search and inspect current and historical orders with line snapshots | Owner and administrator | `/admin/orders`; `getOrdersV2`; `getOrderDetailV2` | `LIVE_UNVERIFIED` | Current route, action, and snapshot coercion paths; Wave 2 audit verifies local ADMIN guards on both reads | No current UI/operator verification for filtering, large-volume behavior, export, mobile cards, or sensitive-field minimization | `orders_v2`, `order_lines_v2`, catalog snapshots | 2026-07-18; repository inspection and access audit | Antigravity (UI); Codex (data); Claude review |
...
| Feature ID | Business capability | Intended users | Current entry points | Status | Evidence | Known limitations | Data affected | Last verified | Owner/maintainer |
|---|---|---|---|---|---|---|---|---|---|
| `RPT-SALES` | Report sales totals and product/modifier detail with date, brand, and category filters | Owner and administrator | `/admin/reports/sales`; `getSalesDataV2` | `LIVE_VERIFIED` | `app/admin/reports/actions.test.ts` sales and ADMIN-rejection cases; `lib/report-time.test.ts`; order-math tests | No CSV/spreadsheet export or scheduled delivery; UI mobile/accessibility operator walkthrough is missing | Read-only orders, lines, products, modifiers | 2026-07-18; repository/tests and access audit | Codex (report math/data); Antigravity (UI); Claude review |
| `RPT-PNL-MAC` | Report revenue, pinned MAC COGS, gross profit, and breakdowns without double counting | Owner and administrator | `/admin/reports/pnl`; `getPnLDataV2`; P&L audit scripts | `LIVE_VERIFIED` | `app/admin/reports/actions.test.ts` including direct ADMIN rejection; `lib/report-v2-allocators.test.ts`; `lib/mac-cogs.test.ts`; `scripts/audit-pnl-mac-consistency.ts`; reviewed MAC audit artifacts | Stored COGS is authoritative while replay differences may be informational; no export/notification; malformed reference data can fail the report rather than silently guess | Orders, lines, snapshots, stock ledger, report aggregates | 2026-07-18; repository/tests and reviewed audits | Codex (financial engine/data); Antigravity (UI); Claude review |
...
| `RPT-STOCK` | View current stock quantities and inventory status | Owner and administrator; POS receives compact stock status only | `/admin/reports/stock`; `getRealtimeStock`; POS `getPOSStockStatus` | `LIVE_UNVERIFIED` | Current ledger-derived stock action; Wave 2 tests verify full ADMIN rejection and narrow authenticated POS shape; stock audit scripts exist | No report-specific operator walkthrough; physical stock is not certified by ledger balance; export and future inventory-role scope remain unverified | Stock ledger and inventory references; compact POS status | 2026-07-18; repository/tests and access audit | Codex (data); Antigravity (UI); Claude review |
| `RPT-PROMOTION-PERFORMANCE` | Compare promotion usage and attributed revenue/discount outcomes | Owner and administrator | Sales/P&L data actions; `getPromotionPerformanceV2` | `LIVE_UNVERIFIED` | Current server action and promotion/order snapshot data; Wave 2 action-guard audit verifies local ADMIN enforcement | No dedicated calculation test, current UI entry confirmation, operator walkthrough, export, or historical-policy review | Orders, lines, promotion snapshots, promotions | 2026-07-18; repository inspection and access audit | Codex (report math/data); Antigravity (UI); Claude review |
