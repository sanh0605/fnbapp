...
| `AUTH-SESSION-AUTHZ` | Restrict protected pages and sensitive actions by session and technical role | Owner and administrator; staff restricted to POS | `middleware.ts`; `resolveActor`; `requireAdmin`; protected `/admin/*` and `/pos/*` routes | `PARTIAL` | `middleware.ts`; `lib/auth.ts`; `lib/admin-auth-guard-audit.test.ts`; Gate 1 action/route regression tests; read-only `scripts/audit-admin-action-auth.ts` checked 19 action files | The Gate 1 backdated-review and maintenance-route gaps now enforce local ADMIN sessions; full RLS, session lifecycle, and broader direct-invocation testing belongs to later audit gates | Sessions and all guarded business records | 2026-07-18; repository/tests | Codex (server paths); Claude (access policy); Antigravity (UI) |
