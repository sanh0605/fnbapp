...
|---|---|---|---|
| [!] **Full audit Gate 3 Phase A — Database/RPC/RLS live audit (read-only)** | Codex | Live Supabase DB: RLS status per table, `anon`/`authenticated` role grants, `exec_sql` RPC grant check, financial-RPC security-definer/grant review, anon-key browser-exposure confirmation | Implemented read-only and awaiting Claude review. Live evidence: 32/32 tables RLS-enabled with zero policies; 10 application RPCs service-role-only; `exec_sql` absent; browser bundle contains no Supabase key. Phase B inputs: broad dormant table grants, orphaned `next_order_num`, and untracked RLS auto-enable event trigger. Report: `docs/audits/2026-07-19-gate3-database-rls-audit.md`. |
...
- `2026-07-19-codex-gate3-database-rpc-rls-audit.md` → Full audit Gate 3 Phase A — implemented by Codex, awaiting Claude review
