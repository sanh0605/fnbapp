...
|---|---|---|---|
| [~X] **Full audit Gate 4 Phase A — Order/inventory/COGS business audit** | Codex | Items 1+1a done; Item 2 in progress (1 of 5 paths tested); Item 3 pending | Handoff: `docs/handoffs/2026-07-19-codex-gate4-order-inventory-cogs-audit.md`. `voidOrderV2` forced-failure test (commit `15e3889`, Claude-verified) found a real gap broader than expected: reversal succeeds + event insert fails → retry silently double-writes the stock reversal (not just the already-known stuck-COMPLETED window). Classified `needs-atomic-rpc`, logged as G4-B1 below (P1). Claude decision: continue evidence collection on the remaining 4 paths before opening any remediation, so Phase B can be scoped as one coherent atomic-RPC effort rather than fixed path-by-path. |
