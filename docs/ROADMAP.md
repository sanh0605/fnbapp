...
|---|---|---|---|
| [~X] **Full audit Gate 4 Phase A — Order/inventory/COGS business audit** | Codex | Items 1+1a done; Item 2 in progress (3 of 5 paths tested); Item 3 pending | Handoff: `docs/handoffs/2026-07-19-codex-gate4-order-inventory-cogs-audit.md`. 3/3 tested paths so far are `needs-atomic-rpc`: `voidOrderV2` (G4-B1), `saveProductionOrder` (G4-B2, worse — no cleanup, no idempotency guard at all, retry silently double-consumes ingredients), `supersedeOrderV2` (preliminary — single failures clean up correctly, but a failed cleanup leaves orphan `Order_Lines_V2` rows that then block all retries). Pattern now looks systemic across the non-atomic write paths. Claude decision: still evidence-only, continue to the last 2 paths (`saveProduct`, stock adjustment) before scoping Phase B as one combined atomic-RPC effort. |
...
| **G4-B3. `supersedeOrderV2` (order edit) — preliminary, needs atomic RPC** | Codex | `lib/sheets-db-v2-edit.ts` `supersedeOrderV2`, 5 sequential writes with best-effort reverse-order cleanup | Gate 4 Item 2 2026-07-19, preliminary (final classification deferred until Item 2 fully closes), Claude-verified (`lib/sheets-db-v2-edit.failure.test.ts`). Single-step failures clean up correctly and retry safely (5/5 tested). But if the cleanup itself also fails, orphan `Order_Lines_V2` rows remain and every subsequent retry fails on a primary-key conflict — a stuck, not silently-duplicating, state requiring manual intervention. |
...
- `2026-07-19-codex-gate4-order-inventory-cogs-audit.md` → Full audit Gate 4 Phase A — ready for Codex pickup, P1
