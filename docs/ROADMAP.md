...
|---|---|---|---|
| (none) | — | — | Gate 4 Phase B merged to `main` 2026-07-19 (owner approved). See `COMPLETED.md`. Not pushed to `origin` yet. |
...
| **G3-A4. Revoke unnecessary table grants for `anon`/`authenticated`** | Codex | Live DB: 28 tables with broad `SELECT/INSERT/UPDATE/DELETE/...` grants to `anon`/`authenticated`, currently blocked only by RLS default-deny | Gate 3 Phase A 2026-07-19, Low severity. Server-only architecture doesn't need these grants at all; retaining them increases blast radius if RLS/a policy is ever misconfigured. Live DB change — needs separately reviewed migration + availability test, not a quick fix. |
| **G3-A5. Resolve orphaned `next_order_num` function** | Codex | Live DB: `SECURITY DEFINER` function, `anon`/`authenticated` executable, no internal caller check, targets a table (`order_counters`) that doesn't currently exist | Gate 3 Phase A 2026-07-19, Medium severity. Dormant now (can't complete its write), but would auto-activate if `order_counters` were ever created. Revoke/drop or redesign before any schema change recreates that table. Live DB change — separately reviewed. |
| **G3-A6. Capture the live RLS auto-enable event trigger in a migration** | Codex | Live DB: `rls_auto_enable()` event trigger explains why all 32 tables have RLS despite no tracked migration enabling it — configuration drift from the migration record | Gate 3 Phase A 2026-07-19, Medium severity (documentation/provenance gap, not an active exposure). Write a migration or policy record capturing this so it isn't a surprise later. |
| **G3-A7/A8. RPC defense-in-depth note + 2 stale diagnostic scripts** | Codex | `exec_sql`/`get_table_constraints` referenced only by 2 old one-off scripts, confirmed absent live | Gate 3 Phase A 2026-07-19, Low severity. A2/A7: current RPC security relies entirely on grant-only enforcement (no internal caller check) — acceptable now, worth adding defense-in-depth if these RPCs are ever touched again. A8: candidate for the `scripts/` cleanup pass already tracked under REV-1's broader context. |
| **G4-B1. `voidOrderV2` needs an atomic RPC** | Codex | `app/admin/orders/actions.ts` `voidOrderV2`, 3 sequential writes (reversal ledger → event → status update), no transaction | Gate 4 Item 2 2026-07-19, P1, Claude-verified (`app/admin/orders/actions.failure.test.ts`). Confirmed: if the reversal batch succeeds but the event insert fails, an operator retry silently writes the stock reversal a second time (not caught by the existing event-based idempotency guard). Real data-integrity risk, conditional on a specific mid-request failure — no evidence this has happened in production. Deliberately not fixed yet: Claude wants the remaining 4 Item-2 paths evidenced first so Phase B can convert whatever needs it to an atomic RPC (matching `create_pos_order_atomic`/`save_purchase_order_atomic`) in one coherent pass. |
| **G4-B2. `saveProductionOrder` needs an atomic RPC — worse than G4-B1** | Codex | `app/admin/production/actions.ts` `saveProductionOrder`, 4 sequential writes, no cleanup, no idempotency guard | Gate 4 Item 2 2026-07-19, P1, Claude-verified (`app/admin/production/actions.failure.test.ts`). Confirmed: if the `PRODUCTION_YIELD` insert fails after ingredients are already consumed, an operator retry creates a whole new production order and consumes the ingredients a second time — silently, since retry reports success. No cleanup and no idempotency guard at all (worse than `voidOrderV2`, which at least partially guards one failure window). Same deferred-remediation reasoning as G4-B1. |
| **G4-B3. `supersedeOrderV2` (order edit) — needs atomic RPC** | Codex | `lib/sheets-db-v2-edit.ts` `supersedeOrderV2`, 5 sequential writes with best-effort reverse-order cleanup | Gate 4 Item 2 2026-07-19, final classification, Claude-verified (`lib/sheets-db-v2-edit.failure.test.ts`). Single-step failures clean up correctly and retry safely (5/5 tested). But if the cleanup itself also fails, orphan `Order_Lines_V2` rows remain and every subsequent retry fails on a primary-key conflict — a stuck, not silently-duplicating, state requiring manual intervention. |
| **G4-B4. `saveProduct` needs an atomic RPC** | Codex | `app/admin/products/actions.ts` `saveProduct`, create path has 4 sequential writes with no cleanup/idempotency key; edit path has independent price/history/recipe/deletion writes | Gate 4 Item 2 2026-07-19, Claude-verified (`app/admin/products/actions.failure.test.ts`, 7 tests). Create: any failure after the first insert leaves an orphan row, and retry always creates a brand-new product/variant rather than completing the old one — duplicate catalog headers. Edit: the decisive finding — a price update that succeeds followed by a price-history insert failure leaves the new price live with no history row; retry sees no price change and writes no history either, so the price-change audit trail is permanently and silently lost (not just delayed, unlike the other 4 paths' stuck/duplicate patterns). |
| **G4-B5. Stock adjustment submit/approve needs an atomic RPC** | Codex | `app/admin/inventory/actions.ts` `submitStockAdjustment`/`approveStockAdjustment`, adjustment-row write then ledger-row write, no transaction | Gate 4 Item 2 2026-07-19, P1, Claude-verified (`app/admin/inventory/actions.failure.test.ts`, 4 tests). Both entry points can leave an adjustment marked `APPROVED` with no corresponding `Stock_Ledger` row if the ledger insert fails: `submitStockAdjustment`'s retry creates a second APPROVED adjustment (duplicate business record, orphan first record); `approveStockAdjustment`'s retry is rejected by its own status guard ("Phiếu đã được duyệt") while the ledger effect never lands — a durable stuck state, approved on paper but with no stock effect. |
...
- **Negative stock recovery** — needs physical count decision from user. Current live state per Gate 4 fresh audit (2026-07-19, `docs/audits/2026-07-19-gate4-correctness-baseline.md`): **3 items negative** — `ING-003` Sữa đặc -131 g (new, started 2026-07-18, independently verified by Claude), `ING-021` -729.821 g (existing), `ING-024` -150 ml (existing). The previously tracked `ING-001`, `NNL-003`, `NNL-006` no longer appear in the live negative list — likely resolved since they were last logged here, not reverified as fixed; don't assume clean without checking if they come up again.
...
- `2026-07-19-codex-gate4-phase-b-atomic-rpc-remediation.md` → Gate 4 Phase B — atomic RPC remediation, ready for Codex pickup, P1
