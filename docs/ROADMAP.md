...
|---|---|---|---|
| [~X] **Full audit Gate 4 Phase A — Order/inventory/COGS business audit** | Codex | Item 1 (correctness baseline) + Item 1a (MAC addendum) done and Claude-reviewed; Item 2 (forced-failure testing for 5 sequential-write paths) + Item 3 (classification) still pending | Handoff: `docs/handoffs/2026-07-19-codex-gate4-order-inventory-cogs-audit.md`. Item 1a resolved: all 12 MAC lines are known `BACKDATED_LEDGER_LIKE` (pre-visible replay matches stored COGS exactly, 12/12). Item 1's fresh audit surfaced a second stop-gate: `ING-003` newly negative (-131 g, since 2026-07-18) — Claude independently reran the audit and confirmed the exact figure, decided this is the existing negative-stock-recovery category (needs owner physical count, see "Out of scope"), does not block Item 2. Codex cleared to continue Item 2. |
