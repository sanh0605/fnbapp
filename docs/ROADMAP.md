...
|---|---|---|---|
| [ ] **Full audit Gate 5 — POS checkout idempotency** | Codex | `app/pos/actions.ts`, `lib/order-cart.ts`, `components/POSScreen.tsx`/`CartPanel.tsx`, `create_pos_order_atomic` migration | Scoped 2026-07-19 after investigation (spec placeholder had no detail; the "offline" half of the original Gate 5 name is explicitly out of scope per owner decision D2, `docs/FEATURE-CATALOG.md`). Real finding: `submitOrderV2` generates a fresh order ID every call and has no server-side idempotency key — a network timeout/dropped connection after the atomic RPC actually commits, followed by a cashier retry, creates a second fully-valid order (double revenue, double stock consumption). Handoff: `docs/handoffs/2026-07-19-codex-gate5-pos-checkout-idempotency.md`. Owner authorized continuous autonomous work tonight (2026-07-19) — Codex should proceed through the whole handoff and REV-1 without waiting for a review checkpoint between steps, only stopping for a genuine stop-gate; Claude merges/reviews everything when the user is back. |
...
- `2026-07-19-codex-rev1-script-fix-crosscheck.md` → REV-1 — cross-check 2 pre-rule script fixes — ready for Codex pickup, P2
