...
- **Xoá hẳn hiện chỉ chặn bằng `requireAdmin()`**, nghĩa là Manager xoá được. Chín chỗ:
  ...
  Cộng thêm `deleteEntity()` ở `lib/db/shared-actions.ts:54` — hàm dùng chung, hiện
  là đường xoá của Thương hiệu (`app/admin/brands/actions.ts:61`) và Nhà cung cấp
  (`app/admin/suppliers/actions.ts:177`). Tổng cộng mười chỗ.
...
| mười file có hàm xoá (bảng ở mục Hiện trạng) | `requireAdmin()` → `requireOwner()` |
...
- Sửa: mười chỗ trong bảng ở mục Hiện trạng
...
const HARD_DELETE_ACTIONS: Array<[string, string]> = [
  ...
  // Shared helper -- this one covers Brands and Suppliers at once.
  ["lib/db/shared-actions.ts", "deleteEntity"],
];
...
Chờ: ĐỎ, mười case đầu hỏng với `expected ... to contain "requireOwner()"`.
Đỏ vì **giá trị sai** — mười hàm đó đang gọi `requireAdmin()`. Hai case cuối xanh
...
`lib/db/shared-actions.ts` là hàm dùng chung của Thương hiệu và Nhà cung cấp; đổi ở
đó siết cả hai màn hình cùng lúc, đúng ý.

- [ ] **Bước 4: Chạy cho thấy xanh**

Chạy: `npx vitest run lib/auth/delete-guard.test.ts` → XANH, 12 test.
...
**Giao diện:**
- Cần từ mục trước: ba bảng ở Mục 1; `parseCashEntry` ở Mục 2.
- Đưa ra cho mục sau: không.
