...
- [ ] **Step 5: Verify the known real case is caught, and the census matches the spec**

Run: `npx vite-node -e "const fs=require('fs');const g=require('glob');import('./scripts/system-map/extract-writes.ts').then(m=>{const files=g.globSync('{app,lib}/**/*.ts').filter(f=>!f.endsWith('.test.ts')&&!f.includes('sheets_db')&&!f.includes('lib/historical')).map(p=>({path:p,source:fs.readFileSync(p,'utf8')}));const r=m.extractWrites(files);console.log('writes:',r.writes.length,'unresolved:',r.unresolved.length);console.log('auth users:',r.writes.find(w=>w.file==='app/actions/auth.ts'));})"`
Expected: a non-zero write count; `auth users:` shows `{ file: 'app/actions/auth.ts', table: 'users' }` (the direct-supabase case the round-9 review found); `unresolved` near 0 (spec §2.24 measured no live 4th shape). If `glob` is unavailable, list files with `fs` recursion instead — do not add a dependency.
...
for (const base of ["app", "lib"]) {
  walk(join(root, base), p => {
    ...
    if (p.includes("sheets_db") || p.includes(join("lib", "historical"))) return;
    codeFiles.push({ path: relative(root, p).split(sep).join("/"), source: readFileSync(p, "utf8") });
  });
}
const tables = extractTables(sql);
const { writes, unresolved } = extractWrites(codeFiles);
...
Run: `npx vite-node scripts/system-map/generate.ts && head -30 docs/generated/system-map.md`
Expected: prints the counts line; the file exists with a `relations` block and a `## Tables` section listing real tables like `orders_v2`, `stock_issues`.
...
**Files:**
- Create: `scripts/doc-checks/map-drift-core.ts`
- Test: `scripts/doc-checks/map-drift-core.test.ts`

**Interfaces:**
- Consumes: `parseRelationBlock` (Task 1), `CheckResult` shape from `check-rules-current-core.ts` (`{ check, ok, problems }`).
- Produces: `checkMapDrift(generatedMarkdown: string, handMarkdown: string): CheckResult`. A relation present in generated but absent from hand → a problem. (One direction only, per spec §3.6: generated is ground truth; the hand map may intentionally omit reads, but must not miss a write relation.)
...
```ts
import { parseRelationBlock } from "../doc-map/relation-block";
import type { CheckResult } from "../check-rules-current-core";
...
Note: `CheckResult` is exported from `scripts/check-rules-current-core.ts`. If importing across `scripts/` subfolders is awkward, re-export the type from a shared `scripts/check-result.ts` and import from there in both places (small refactor, no behavior change).
...
The Phase-1 "done" bar is a real catch, not a synthetic fixture. Pick `stock-issue` (a flow whose write-sites the generator already emits). Write the hand map block to match the generated one for that flow, confirm the check passes, then delete one real relation from the hand block and confirm it fails.
...
- [ ] **Step 3: Write a minimal seed workflow doc**

Create `docs/03-workflows/stock-issue.md` with the declaration block Task 7 will check, filled from real files (`app/admin/inventory/issue-slips/actions.ts`, table `stock_issues`). Keep it under the 200-line ceiling.
...
describe("seed proof: drift check catches a real removed relation", () => {
  ...
  it("passes on the committed seed", () => {
    // The hand map need only cover the relations it claims to; restrict the
    // generated side to stock_issues so the seed proof is scoped to the seed.
    const genStockOnly = generated.split("\n").filter(l => l.includes("stock_issues") || l.includes("```")).join("\n");
    expect(checkMapDrift(genStockOnly, hand).ok).toBe(true);
  });
  ...
  it("fails when a real relation is dropped from the hand map", () => {
    ...
    const genStockOnly = generated.split("\n").filter(l => l.includes("stock_issues") || l.includes("```")).join("\n");
    const r = checkMapDrift(genStockOnly, brokenHand);
    expect(r.ok).toBe(false);
...
const decl: FlowDecl = {
  ...
  files: ["app/admin/inventory/issue-slips/actions.ts"],
  tables: ["stock_issues"],
  brCodes: ["BR-COGS-005"],
};
...
describe("checkFlowFacts", () => {
  it("passes when every declared fact matches reality", () => {
    const r = checkFlowFacts(decl, {
      ...
      files: new Set(["app/admin/inventory/issue-slips/actions.ts"]),
      writesByFile: new Map([["app/admin/inventory/issue-slips/actions.ts", new Set(["stock_issues"])]]),
  ...
  it("fails when a declared table is not actually written by the declared files", () => {
    const r = checkFlowFacts(decl, {
      routes: new Set(["/admin/inventory/issue-slips"]),
      files: new Set(["app/admin/inventory/issue-slips/actions.ts"]),
      writesByFile: new Map([["app/admin/inventory/issue-slips/actions.ts", new Set()]]),
      brCodes: new Set(["BR-COGS-005"]),
    });
    expect(r.ok).toBe(false);
    expect(r.problems.join(" ")).toContain("stock_issues");
...
describe("checkFlowStagedCoupling", () => {
  it("fails when a flow's source file is staged but its doc is not", () => {
    const r = checkFlowStagedCoupling([decl], ["app/admin/inventory/issue-slips/actions.ts"]);
    expect(r.ok).toBe(false);
  ...
  it("passes when both the source file and its doc are staged", () => {
    const r = checkFlowStagedCoupling([decl],
      ["app/admin/inventory/issue-slips/actions.ts", "docs/03-workflows/stock-issue.md"]);
    expect(r.ok).toBe(true);
  });
});
```
...
Also implement `parseFlowDecl` (reads a fenced `flow-decl` block of `key: comma,separated,values` lines) alongside — same fenced-block technique as Task 1. Add a focused test for it.
...
Create `scripts/doc-checks/open-items.ts` that runs `npx vitest run --reporter=json --outputFile=<tmp>` (or captures stdout), parses `testResults[].assertionResults[]` for `status === "todo"`, maps to `{title, file}`, and writes `docs/04-operations/OPEN-ITEMS.md` via `renderOpenItems`. Then add a temporary probe test `lib/__todo_probe.test.ts` with `it.todo("probe")`, run the CLI, confirm the probe appears in the output file, then delete the probe.
...
**Spec coverage:**
- §3.5b write extraction incl. direct supabase.from → Task 3. ✓
...
**Type consistency:** `CheckResult` is the shared shape (`scripts/check-result.ts`, re-exported from `check-rules-current-core.ts`); `Relation`, `TableInfo`, `WriteSite`/`UnresolvedWrite`, `RouteActions`, `FlowDecl` are each defined once and consumed by name in later tasks. `checkMapDrift`, `checkFlowFacts`, `checkFlowStagedCoupling`, `checkLineCeiling`, `renderOpenItems`, `buildMap`, `extractTables`, `extractWrites`, `extractRoutes`, `parseRelationBlock`/`serializeRelations` — names match across tasks.
