...
## 2026-07-30 - CLOSED, 6/6 tasks, applied: clean rebuild program, Phase 6 (recipe snapshot repair)

**Done 2026-07-30.** Owner-directed investigation into why `recipe_snapshot_json` sometimes disagreed with the recipe in force at sale time, root-caused to two independent bugs (order-edit resolving against the edit's own timestamp, and the V1→V2 migration hand-rolling a filter instead of calling `selectEffectiveRecipe`) — both fixed with regression tests. Repaired 237/237 corrupted historical snapshots, then re-ran Phase 4 (stock, +5,770 g on BTP-008) and Phase 5 (cost, -158,760 VND / +158,778 VND profit) on the corrected basis. Owner demanded and got a full arithmetic reconciliation of the BTP-008 swing before allowing `--apply`: it matches the repair exactly via an isolated full-history replay, even though a naive per-line formula sum gives the opposite sign (explained by implicit-production batch path-dependency, not an error). Patched 3 anti-recurrence holes so this class of bug can't happen again: the backdated-recipe trigger now fires on UPDATE and watches `start_date` (migration `0043`); recovery repairs the snapshot before recomputing cost, not cost alone; every new recipe now writes `start_date` (migration `0044` fixes `save_product_atomic()`'s RPC insert — the plan's file list pointed at the wrong file). Final verify clean across the board: 0 repairable snapshot mismatches, `audit-full-history-recompute.ts` 0/0/0/0 across all 4 sections. **Running total, historical profit revised by this rebuild program: Phase 5 +942,492 VND + this repair +158,778 VND = +1,101,270 VND.** Full writeup: `DEVELOPMENT-TRACKING.md` same-date entry "Phase 6: Recipe Snapshot Repair". Roadmap row: `REBUILD-PHASE6`. Deferred: backfilling `start_date` on 129 pre-existing null rows, and Task 7 (backup pipeline never received Phase 3's coverage fix) — neither started.
