...
| 34 | **`stock_issues` is not backed up** | `supabase/functions/backup-to-drive/core.ts`'s `BACKUP_TABLES` predates Plan B's migration `0052` and was never extended to include the new table. Harmless today — `stock_issues` holds 0 rows, confirmed during Plan C Task 1's restore drill (2026-08-05). Becomes real the day the owner runs the first stocktake: every issue event recorded after that point exists only in production, with no copy in the daily Drive backup or any restore drill. Add `stock_issues` to `BACKUP_TABLES` before the first real count, not after. |
