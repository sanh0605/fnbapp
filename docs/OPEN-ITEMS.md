...
| 28 | **Phase 4 — UI/UX rules** | Deferred by owner decision until after the restructure. 28 pages, 15+ inconsistent empty-state patterns measured 2026-07-06. **Gained a standard 2026-08-08:** the owner made mobile-first mandatory for every page including layout (`CLAUDE.md` section 8), so this phase now has a target to converge on rather than only a list of inconsistencies. New and touched pages follow it immediately; the existing 28 are this item's job and are not a licence to rewrite the admin in one pass. Measured that day: `PurchaseOrdersClient` 22 responsive rules and `PurchaseOrderForm` 12, against 4 and 1 in the two screens Plan D had just built — the house standard already exists, it is applied unevenly. |
