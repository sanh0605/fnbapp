...
| 30 | **The completed-purchase path still turns an unusable rate into zero** | Successor to the closed item 29, which only ever named `lib/purchase-order-write-plan.ts:92-94` — the draft branch. That branch is now guarded (`fd811c9`), and it holds **0 rows**: there are no DRAFT purchase orders. Every real purchase takes the completed branch, which computes base quantity in `lib/purchase-ledger-rebuild.ts:63-65` as `quantity * (Number(conversion.conversion_rate) \|\| 0)` with no refusal. `resolveConversion` throws when a conversion cannot be found, so the remaining triggers are a stored rate of 0 and a line quantity of 0 against a real subtotal. `uom_conversions.conversion_rate` is `numeric(18,6) not null` with **no positivity check**, so a zero rate is storable. Measured 2026-08-04: 0 of 57 conversion rows are unusable, so this is latent, not live. A hardened implementation of the same arithmetic already exists — `lib/purchase-line-base-quantity.ts` refuses both cases — and is called by exactly one backfill script, not by the write path. Closed by Plan B Task 1d. |
| 31 | **The financial report needs redesigning, and it is not a P&L today** | Owner decision 2026-08-04: discuss after the COGS change lands. Two findings recorded so they are not rediscovered. First, `app/admin/reports/pnl/page.tsx` is titled "Báo cáo Lãi Lỗ (P&L)" but stops at gross profit — there is **no operating-expense table anywhere in the schema**, so rent, wages, utilities and packaging have never been subtracted. July 2026 reads "lãi 11.412.736đ" before any of them. Second, issue-based costing removes the per-drink cost breakdown outright and forces the ingredient-consumption table to be rebuilt around purchased items; Plan C Task 2 carries that forced part. Expense tracking is a separate subsystem and needs its own spec. |
