...
| 35 | **`STK-` prefix names two different id spaces** | Stocktake session ids (`open_stocktake_session_atomic`) and `stock_ledger` row ids (`apply_stocktake_session_atomic`) both use the `STK-` prefix, each numbered from `max()` of its own table — not a collision (different tables, never compared to each other), but two unrelated things wearing the same name. Noted while building D12 (2026-08-09), not fixed there — a rename touches every place either id is displayed or matched by pattern. |
