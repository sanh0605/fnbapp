-- Atomic recovery for one trigger-captured backdated recipe-version event.
-- Mirrors 0015_backdated_event_recompute.sql's design exactly, adapted to
-- read/write backdated_recipe_events instead of backdated_ledger_events.
-- Declares search_path with `extensions` from the start (0015 originally
-- omitted it and needed a same-day fix in migration 0026, because
...
create or replace function public.mark_backdated_recipe_event_recomputed(
  p_event_id uuid,
  p_reviewer text,
  p_run_id text,
...
begin
  ...
  if p_change_count is null or p_change_count < 0 then
    raise exception 'p_change_count must be non-negative';
  end if;
  ...
  update public.backdated_recipe_events
...
create or replace function public.reject_backdated_recipe_event(
  p_event_id uuid,
  p_reviewer text,
  p_reason text
)
...
revoke all on function public.reject_backdated_recipe_event(uuid, text, text) from public;
revoke all on function public.reject_backdated_recipe_event(uuid, text, text) from anon;
revoke all on function public.reject_backdated_recipe_event(uuid, text, text) from authenticated;
grant execute on function public.reject_backdated_recipe_event(uuid, text, text) to service_role;
...
revoke all on function public.mark_backdated_recipe_event_recomputed(uuid, text, text, integer) from public;
revoke all on function public.mark_backdated_recipe_event_recomputed(uuid, text, text, integer) from anon;
revoke all on function public.mark_backdated_recipe_event_recomputed(uuid, text, text, integer) from authenticated;
grant execute on function public.mark_backdated_recipe_event_recomputed(uuid, text, text, integer) to service_role;
