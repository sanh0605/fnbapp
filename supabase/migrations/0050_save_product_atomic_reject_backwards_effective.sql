-- Task 6 (2026-07-31 recipe-start-date-backfill-and-not-null plan): a recipe
-- cannot stop applying before it starts. save_product_atomic's CREATE_VERSION
-- path closes the superseded recipe with end_date = coalesce(p_effective_at,
-- now()) without checking that value against the superseded recipe's own
-- start_date, so an effective date typed earlier than the recipe it replaces
-- produces end_date < start_date -- an interval with no moment where the
-- recipe was ever actually effective. app/admin/semi-products/actions.ts
-- already refuses this (same task, JS save path); this closes the matching
-- gap in the RPC that serves PRODUCT_VARIANT recipes.
--
-- Owner decision, 2026-07-31: refuse rather than auto-adjust. The situation
-- always means a date was mistyped -- either the superseded recipe's start
-- or the new effective date -- and only the operator knows which.
--
-- Everything else is copied unchanged from 0044.
...
begin
  ...
  loop
    ...
    if v_recipe_decision = 'UNCHANGED' then
      ...
      -- A recipe cannot stop applying before it starts. Reaching here with an
      -- effective date earlier than the superseded recipe's own start_date
      -- means a date was mistyped -- either that recipe's start or this save's
    ...
    if v_recipe_decision in ('CREATE_INITIAL', 'CREATE_VERSION') then
      insert into public.recipes (
        id, target_type, target_id, ingredients_json, start_date, end_date, status,
