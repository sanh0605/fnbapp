-- Support paying for one order with more than one payment method (e.g. part
-- cash, part bank transfer), confirmed to happen in practice
-- (docs/audits/2026-07-20-feature-completeness-fnb-checklist-reconciliation.md).
-- orders_v2.payment_method/payment_ref stay as-is for backward compatibility
-- with existing rows and reports; order_payments becomes the detailed,
-- per-method source of truth for every order going forward (exactly one row
-- for the ordinary single-method case).

create table if not exists public.order_payments (
  id text primary key,
  order_id text not null references public.orders_v2(id),
...
begin
  ...
  -- Backward compatibility: a caller that hasn't been updated to send
  -- p_payments yet gets a single synthesized payment row from the legacy
  -- payment_method/payment_ref/net_total fields, so existing behavior is
  -- unchanged until every caller passes payments explicitly.
