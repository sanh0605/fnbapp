...
revoke all on table public.data_migration_runs from public, anon, authenticated;
grant select, insert on table public.data_migration_runs to service_role;
...
begin
  ...
  if v_existing.migration_key is not null then
    ...
    if exists (
      ...
      left join public.stock_ledger ledger
        on ledger.id = expected->>'id'
      where
        ledger.id is null
        or ledger.reference_id <> expected->>'reference_id'
        or ledger.item_reference <> expected->>'item_reference'
        or ledger.transaction_type <> expected->>'transaction_type'
        or ledger.quantity_change <> (expected->>'quantity_change')::numeric
        or ledger.unit_cost <> (expected->>'unit_cost')::numeric
        or ledger.created_at <> (expected->>'created_at')::timestamptz
        or coalesce(ledger.order_event_id, '') <>
          coalesce(expected->>'order_event_id', '')
        or ledger.cost_at_sale <> (expected->>'cost_at_sale')::numeric
        or coalesce(ledger.source, '') <> coalesce(expected->>'source', '')
        or coalesce(ledger.notes, '') <> coalesce(expected->>'notes', '')
    ) then
