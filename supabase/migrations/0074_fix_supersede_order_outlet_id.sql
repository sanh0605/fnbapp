-- Fixes docs/superpowers/plans/2026-08-27-fix-order-edit-outlet-id.md
-- (OPEN-ITEMS: order editing broken since 2026-08-25).
--
-- 0072 tightened orders_v2.outlet_id to NOT NULL. supersede_order_v2_atomic
-- was last defined in 0046, before that column existed -- its insert into
-- orders_v2 never named outlet_id, so every edit since 0072 landed raises
-- "null value in column \"outlet_id\" ... violates not-null constraint".
--
-- Sourced from the order being superseded (the SELECT that already reads
-- its status/version, extended to also read outlet_id), not from
-- p_new_order: an edit never moves an order to a different outlet, and
-- reading it from the row being edited means the client cannot omit or
-- contradict it -- the exact class of bug this migration exists to fix.
--
-- Copied forward from 0046 unchanged apart from that one addition.
...
-- Same signature as 0046's -- create or replace preserves existing grants,
-- but restated for defensiveness (harmless no-op if already in force),
-- matching 0046's own pattern.
