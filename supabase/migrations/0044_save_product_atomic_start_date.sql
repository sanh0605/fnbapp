-- Widens save_product_atomic() (0021_atomic_product_save.sql) to write
-- start_date on every recipes row it creates for a PRODUCT_VARIANT, not
-- only created_at. Same origin as the semi-product and modifier save paths
-- (2026-07-30 phase6 plan, Task 5 Step 5): effectiveness is decided by
-- start_date, falling back to created_at (lib/recipe-selection.ts's
-- selectEffectiveRecipe) -- leaving start_date null makes 0043's
-- backdated-recipe trigger read created_at, which can differ from the
-- effective date the caller (app/admin/products/actions.ts) actually
-- passed as p_effective_at.
--
-- Only the recipes insert changes: start_date is added alongside end_date,
-- set to the same coalesce(p_effective_at, now()) already used for
-- created_at. Everything else is copied unchanged from 0021.
