-- Migration 0012 was already recorded remotely before its recovery RPC and
-- lock protections were hardened. Reapply those definitions as a forward
-- migration so deployed databases receive the reviewed behavior.
...
drop function if exists public.apply_mac_drift_recovery(jsonb, text, text);
